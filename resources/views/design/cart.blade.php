...
<div class="card_container py-3">
    ...
    <form action="{{route('post_cart')}}" method="post" id="payment-form">
        ...
        <div class="list_button"><a href="{{ route('delete_cart',['id'=> $download->id]) }}">削除</a></div>
        </tr>
        @endforeach
        
        <label for="price">合計金額  {{$total}}円</label>
        @if(isset($paymentMethods) && count($paymentMethods) > 0)
            <label for="paymentMethods">登録済みの支払い方法：</label>
            <select name="paymentMethod" id="paymentMethod">
                @foreach($paymentMethods as $paymentMethod)
                    <option value="{{ $paymentMethod->id }}">{{ $paymentMethod->card->brand }} **** **** **** {{ $paymentMethod->card->last4 }}</option>
                @endforeach
            </select>
        @else
