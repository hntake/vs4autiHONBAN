...
<head>
    ...
    <div class="picture">
        ...
        <ul class="icon">
                ...
                <li>
                    <button class="img_button2" style="float:unset;"><a href="{{ route('independence_two',['id'=> $schedule->id]) }}"><img src="{{ asset('img/right-arrow.png')}}"></a></button>
                </li>
        </ul>
            <ul class="icon">
                <li>
...
<script>
        function readAloud1() {
          // 発言を作成
        const uttr = new SpeechSynthesisUtterance("{{$schedule->explain1}}")
        // 発言を再生 (発言キューに発言を追加)
...
<script>
    function readAloud2() {
        // 発言を作成
        const uttr = new SpeechSynthesisUtterance("{{$schedule->caution1}}")
        // 発言を再生 (発言キューに発言を追加)
        speechSynthesis.speak(uttr)
