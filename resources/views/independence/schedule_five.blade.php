...
<div class="picture">
        <img src="{{ asset('storage/' . $schedule->image5) }}" alt="image" >
        <ul class="icon">
                <li>
...
<script>
        function readAloud1() {
          // 発言を作成
        const uttr = new SpeechSynthesisUtterance("{{$schedule->explain5}}")
        // 発言を再生 (発言キューに発言を追加)
...
<script>
    function readAloud2() {
        // 発言を作成
        const uttr = new SpeechSynthesisUtterance("{{$schedule->caution5}}")
        // 発言を再生 (発言キューに発言を追加)
        speechSynthesis.speak(uttr)
