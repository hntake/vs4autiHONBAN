...
<head>
    <meta charset="UTF-8">
    <title>自閉症・発達障害専門掲示板 コメント入力ページ</title>
    ...
    <div class="wrap">
        <main>
            ...
            <a href="{{route('bbs_list')}}">トップページに戻る</a>
                ...
                <h1>自閉症・発達障害専門掲示板</h1>
                <td><a href="{{route('bbs_index',['id'=>$comment->thread_id])}}">スレッド名：{{ $comment->title }}</a></td>
                <h4>コメント入力</h4>
            </div>
            <div class="list">
                <form id="myForm" method="POST" action="{{ route('bbs_post',['id'=> $comment->thread_id]) }}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <table>
                                ...
                                <input type="hidden"size="35" name="title" value="{{$comment->title}}">
